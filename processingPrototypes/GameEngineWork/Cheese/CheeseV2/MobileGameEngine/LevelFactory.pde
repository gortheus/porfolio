...
class LevelFactory
{
  ...
  IGameLevel getLevel(int _levelNumber)
  {
    ...
    if (levelType == 0)
    {
      ...
      level = new SplashScreenLevel(levelNumber, successTargetLevel, failureTargetLevel);
       ...
       level = new AvoidLevel(levelNumber, successTargetLevel, failureTargetLevel);
       configureAlienLevel(configLines, (AvoidLevel)level);
    }else
    {
   ...
   */
  private void configureAlienLevel(ArrayList<String> config, AvoidLevel clevel)
  {
    ...
    for (int i = 3; i < config.size(); i++)//configLineCount > 2)
    {
      ...
      if (pieces.length == 5) {
        ...
        int eh = int(pieces[4]) ;
        if (type == 0)
          clevel.addAlien(ex, ey, ew, eh);
        else if (type == 3)
          clevel.addWall(ex, ey, ew, eh);
        else if (type == 2)
          clevel.addCheese(ex, ey, ew, eh);
        else if (type == 1)
          clevel.addCoin(ex, ey, ew, eh);
        else if (type == 4)
          clevel.addLaser(ex, ey, ew, eh);
